...
program_function='import_postgres_premigration_Errorfile'
program_run_date=''
program_end_date=''
postgres_columns_names = ''

with open('postgres_import_errorfile_cofig.txt', 'r') as f:
    ...
    while text !="":
        ...
        sql = f'''COPY {tab_name}({postgres_columns_names}) FROM '/apps/opt/application/pscmigration/1pscmigration/scripts/phase1_extract/Error_file/{csv_file_name}' delimiter ',' csv header;'''
        #print(sql)
        cursor.execute(sql)
        #read counts
        ...
        text=f.readline()
#print(csv_log)
with open("log/csv_Errorfile_import_to_postgres.csv", 'w',encoding='utf-8', newline='') as f:
    f.write(csv_log)
