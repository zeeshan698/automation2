...
with open('postgres_import_cofig.txt', 'r') as f:
    ...
    while text !="":
        ...
        sql = f'''COPY {tab_name} FROM '/apps/opt/application/pscmigration/1pscmigration/scripts/phase1_extract/data/{csv_file_name}' delimiter ',' csv header;'''
        #print(sql)
        cursor.execute(sql)
        #read counts
        ...
        text=f.readline()
#print(csv_log)
with open("log/csv_import_to_postgres.csv", 'w',encoding='utf-8', newline='') as f:
    f.write(csv_log)
